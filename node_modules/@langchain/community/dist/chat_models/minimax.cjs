 ...
 * ```
 */
class ChatMinimax extends chat_models_1.BaseChatModel {
    static lc_name() {
        return "ChatMinimax";
    }
    get callKeys() {
    ...
    constructor(fields) {
        ...
        this.minimaxGroupId =
            fields?.minimaxGroupId ?? (0, env_1.getEnvironmentVariable)("MINIMAX_GROUP_ID");
        if (!this.minimaxGroupId) {
            throw new Error("Minimax GroupID not found");
        }
        this.minimaxApiKey =
            fields?.minimaxApiKey ?? (0, env_1.getEnvironmentVariable)("MINIMAX_API_KEY");
    ...
    invocationParams(options) {
        return {
            ...
            functions: options?.functions ??
                (options?.tools
                    ? options?.tools.map(openai_1.formatToOpenAIFunction)
                    : undefined),
...
function minimaxResponseToChatMessage(message) {
    switch (message.sender_type) {
        ...
        case "FUNCTION":
            return new messages_1.AIMessage(message.text || "");
        default:
            return new messages_1.ChatMessage(message.text || "", message.sender_type ?? "unknown");
    }
}
